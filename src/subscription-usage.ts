...
export function subscriptionCredentialContext(provider: AcpProvider, env: NodeJS.ProcessEnv): string {
  ...
  const keys = provider === "grok"
    ? ["GROK_CODE_XAI_API_KEY", "XAI_API_KEY", "GROK_CODE_BASE_URL"]
  ...
  try { hash.update(readFileSync(path.join(root, provider === "grok" ? "auth.json" : ".credentials.json"))); }
  catch (error) {
